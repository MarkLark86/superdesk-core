        ...
        try:
            ...
            if not original_mongo and search_backend:
                ...
                if update_identical:
                    ...
                    logger.warning('Item : {} not updated in collection {} as the updates are identical '
                                 'to the item in mongo. Updates are : {}'.format(id, endpoint_name, updates))
                elif original != original_mongo:
                    logger.warning('Original item provided does not match item in mongo for {} in collection {}. '
